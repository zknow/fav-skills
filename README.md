# My Favorite Skills Repositories (我喜愛的技能庫紀錄)

這份文件用於記錄與整理我喜愛的技能庫（Skills Repositories）以及相關之 AI Agent 工具，方便日後查閱與擴充。
...
為了方便您將 [favorite_skills.md](./favorite_skills.md) 中喜愛的技能安裝到特定的 AI Agent 目錄下，專案中附帶了一個**免安裝任何第三方依賴**的互動式終端小工具 [install-skills.js](./install-skills.js)。
...
# 或者，安裝到指定的工作區目錄（例如：D:\my-other-project）
node install-skills.js "D:\my-other-project"
```

#### 2. 跨專案執行方法 (最推薦)：
當您將此專案推送到您的 GitHub（假設倉庫名稱為 `fav-skills`）後，您可以在**任何其他專案的根目錄**直接執行以下指令，無須複製本專案的程式碼：
```bash
npx github:zknow/fav-skills
```
> **提示**：`npx` 會自動下載您的喜愛清單與指令，並在您當前執行指令的專案路徑下執行安裝。
...
- **skills**: 該倉庫中喜愛技能的清單，為一個物件陣列：
  ...
  - **url**: 技能的 GitHub 目錄或檔案之精確連結（例如 `"https://github.com/mattpocock/skills/blob/main/skills/productivity/grill-me/SKILL.md"`）。
- **summary**: 簡短的特色與功能總結。
